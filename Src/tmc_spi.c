...
#if TRINAMIC_SPI_PORT == 11 || TRINAMIC_SPI_PORT == 12
...
uint8_t _spi_put_byte (uint8_t tx_byte)
{
    uint8_t rx_byte;
...
TMC_spi_status_t tmc_spi_read (trinamic_motor_t driver, TMC_spi_datagram_t *reg)
{
...
#ifdef TRINAMIC_SPI_PORT
    DIGITAL_OUT(cs.port, 1 << cs.pin, 0);
#else
    spi_select(&dev);
#endif
    ...
    delay(TRIMANIC_SPI_DELAY*2);
#ifdef TRINAMIC_SPI_PORT
    DIGITAL_OUT(cs.port, 1 << cs.pin, 1);
#else
    spi_deselect(&dev);
#endif
    delay(TRIMANIC_SPI_DELAY);
#ifdef TRINAMIC_SPI_PORT
    DIGITAL_OUT(cs.port, 1 << cs.pin, 0);
#else
    spi_select(&dev);
...
#ifdef TRINAMIC_SPI_PORT
    while(!__HAL_SPI_GET_FLAG(&spi_port, SPI_FLAG_EOT)) {};             // untested...
    DIGITAL_OUT(cs[driver.id].port, 1 << cs[driver.id].pin, 0);
#else
    dev.cs_pin = cs[driver.id].pin;
    dev.cs_port = cs[driver.id].port;
